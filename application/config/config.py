...
class ConfigReader:
    'help to read data from config.ini'
    configFilePath = configparser.ConfigParser()

    def __init__(self, configFilePath):
        self.configFilePath.read(configFilePath)

    def getOneConfig(self, sectionName, configName):
        configContent = self.configFilePath.get(sectionName, configName)
        return configContent

    def getConfigs(self, sections):
        sectionContent = []
        for section in sections:
            configContent = []
            sectionName = section[0]
            for config in section[1:]:
                configContent.append(self.getOneConfig(sectionName, config))
            sectionContent.append(configContent)
